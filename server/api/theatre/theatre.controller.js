 ...
 * GET     /api/theatres              ->  index
 * POST    /api/theatres              ->  create
 * GET     /api/theatres/:id          ->  show
 * PUT     /api/theatres/:id          ->  update
 * DELETE  /api/theatres/:id          ->  destroy
 */
...
export function cityName(req, res) {
  return Theatre.find({state: req.params.state}).exec()
...
export function getseat(req, res) {
  return Theatre.find({state: req.params.state, location:req.params.city}).exec()
    .then(handleEntityNotFound(res))
    .then(respondWithResult(res))
    .catch(handleError(res));
}
